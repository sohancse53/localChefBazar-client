...
const MyMealCard = ({ meal, refetch }) => {
  ...
  const handleMealDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/meal/${meal?._id}`).then((res) => {
  ...
  return (
    <div className="flex flex-col bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden border border-primary/10">
      {/* Food Image */}
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="text-sm text-gray-700 border-t border-primary/10 pt-3">
          <p>
            <span className="font-medium text-primary">Chef:</span>{" "}
            {meal.chefName}
          </p>
          <p>
            <span className="font-medium text-primary">Chef ID:</span>{" "}
            {meal.chefId}
          </p>
        </div>
